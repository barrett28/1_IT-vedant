...
from . forms import Register, ChangeUserDetails, ChangeAdminDetails
    ...
    if request.user.is_authenticated:
        if request.method == 'POST':
            if request.user.is_superuser == True:
                mf= ChangeAdminDetails(request.POST,instance=request.user)
            else:
                mf=ChangeUserDetails(request.POST,instance=request.user)
            if mf.is_valid():
                mf.save()
                return redirect('/profile/')
        else:
            if request.user.is_superuser == True:
                mf= ChangeAdminDetails(instance=request.user)
            else:
                mf=ChangeUserDetails(instance=request.user)
